 ...
 */
class MXFixedFileStore(
    hsConfig: HomeServerConnectionConfig,
    enableFileEncryption: Boolean,
    context: Context
) : MXFileStore(hsConfig, enableFileEncryption, context) {
    private val handlerThread: HandlerThread

    init {
        handlerThread = HandlerThread(
            "MXFileStoreBackgroundThread_" + hsConfig.credentials.userId,
            Thread.MIN_PRIORITY
        )
        handlerThread.start()
