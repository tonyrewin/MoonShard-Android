...
public class VCardCustomManager extends Manager {
    public static final String NAMESPACE = VCard.NAMESPACE;
    public static final String ELEMENT = VCard.ELEMENT;

    private static final Map<XMPPConnection, VCardCustomManager> INSTANCES = new WeakHashMap<>();
     ...
     */
    public static synchronized VCardCustomManager getInstanceFor(XMPPConnection connection) {
        VCardCustomManager vcardManager = INSTANCES.get(connection);
        if (vcardManager == null) {
            vcardManager = new VCardCustomManager(connection);
    ...
    private VCardCustomManager(XMPPConnection connection) {
        super(connection);
    ...
    public VCard loadVCardMuc(EntityBareJid bareJid) throws SmackException.NoResponseException, XMPPException.XMPPErrorException, SmackException.NotConnectedException, InterruptedException {
        VCard vcardRequest = new VCard();
        vcardRequest.setTo(bareJid);
        VCard result = connection().createStanzaCollectorAndSend(vcardRequest).nextResultOrThrow();
        return result;
